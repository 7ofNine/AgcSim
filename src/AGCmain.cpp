...
char* getCommand(char const * prompt)
{
	static char s[80];
	char* sp = s;
...
void toggleBreakpoint()
{
	...
	{
		char b[80];
		strcpy_s(b, 80, getCommand("Set breakpoint: -- enter 14-bit CADR (octal): "));
		cout << endl;
...
void toggleWatch()
{
	...
	{
		char b[80];
		strcpy_s(b, 80, getCommand("Set watch: -- enter 14-bit CADR (octal): "));
		cout << endl;
		...
		char buf[100];
		sprintf_s(buf, 100,"%06o:  %06o", watchAddr, oldWatchValue);
		cout << buf << endl;
	}
	else
	{
...
{
	char cntrname[80];
	strcpy_s(cntrname, 80, getCommand("Increment counter: -- enter pcell (0-19): "));
	cout << endl;
...
{
	char cntrname[80];
	strcpy_s(cntrname, 80, getCommand("Decrement counter: -- enter pcell (0-19): "));
	cout << endl;
...
{
	char iname[80];
	strcpy_s(iname, 80, getCommand("Interrupt: -- enter priority (1-5): "));
	cout << endl;
...
void loadMemory()
{
	strcpy_s(filename, 80, getCommand("Load Memory -- enter filename: "));
	cout << endl;

		// Add the .obj extension.
	char fname[80];
	strcpy_s(fname, 80, filename);
	strcat_s(fname, 80, ".obj");


    FILE* fp = NULL;
    int error = fopen_s(&fp, fname, "r");
	if(error != 0)
	{
		perror("fopen failed:");
	...
	unsigned addr;
	unsigned data;
	while(fscanf_s(fp, "%o %o", &addr, &data) != EOF)
	{
...
{
    FILE* fp = NULL;
    int error = fopen_s(&fp, filename, "w");
	if(error != 0)
	{
...
{
	char theAddress[20];
	strcpy_s(theAddress, 20, getCommand("Examine Memory -- enter address (octal): "));
	cout << endl;
	...
	{
		sprintf_s(buf, 100, "%06o:  %06o", i, MEM::readMemory(i));
		cout << buf << endl;
	}
}
...
void showSourceCode()
{
	...
	char CADR[colLen+1];
    sprintf_s(CADR, colLen+1,"%05o", ADR::getEffectiveAddress());
	...
	while(!feof(fp))
	{
		...
		if(fgets(s, 256, fp))
		{
			...
			strncpy_s(valString, 20, s+startCol, colLen);
			valString[colLen]='\0';
			...
			if(foundit)
			{
				...
				strncpy_s(out, 256, s, maxLineLen);
				out[maxLineLen] = '\0';
				cout << out;
...
int main(int argc, char* argv[])
{
	...
	while(1)
	{
		...
		// DSKY:
		case '0': KBD::keypress(KEYIN_0);
				break;
		case '1': KBD::keypress(KEYIN_1);
				break;
		case '2': KBD::keypress(KEYIN_2);
				break;
		case '3': KBD::keypress(KEYIN_3);
				break;
		case '4': KBD::keypress(KEYIN_4);
				break;
		case '5': KBD::keypress(KEYIN_5);
				break;
		case '6': KBD::keypress(KEYIN_6);
				break;
		case '7': KBD::keypress(KEYIN_7);
				break;
		case '8': KBD::keypress(KEYIN_8);
				break;
		case '9': KBD::keypress(KEYIN_9);
				break;
		case '+': KBD::keypress(KEYIN_PLUS);
				break;
		case '-': KBD::keypress(KEYIN_MINUS);
				break;
		case '.': KBD::keypress(KEYIN_CLEAR);
				break;
		case '/': KBD::keypress(KEYIN_VERB);
				break;
		case '*': KBD::keypress(KEYIN_NOUN);
				break;
		case 'g': KBD::keypress(KEYIN_KEY_RELEASE);
				break;
		case 'h': KBD::keypress(KEYIN_ERROR_RESET);
				break;
		case 'j': KBD::keypress(KEYIN_ENTER);
				break;
