...
void MON::displayAGC()
{
	...
	sprintf_s(buf, 100, " TP: %-5s  F17:%1d   F13:%1d   F10:%1d   SCL:%06o", 
	...
	sprintf_s(buf, 100,"    STA:%01o   STB:%01o   BR1:%01o   BR2:%01o   SNI:%01o   CI:%01o   LOOPCTR:%01o", 
	...
	sprintf_s(buf, 100, " RPCELL:%05o  INH1:%01o  INH:%01o  UpCELL:%03o  DnCELL:%03o  SQ:%02o  %-6s  %-6s", 
	...
	sprintf_s(buf, 100, "     CP:%s", SEQ::getControlPulses());
	cout << buf << endl;

		// For the G register, bit 15 comes from register G15; the other bits (16, 14-1) come
		// from register G.
	sprintf_s(buf, 100,"      S:  %04o   G:%06o   P:%06o   (r)RUN :%1d  (p)PURST:%1d (F2,F4)FCLK:%1d", 
		ADR::register_S.read(), 
	...
	sprintf_s(buf, 100, "    RBU:%06o WBU:%06o  P2:%01o        (s)STEP:%1d", 
	...
	sprintf_s(buf, 100, "      B:%06o           CADR:%06o   (n)INST:%1d      PALM:[%c]", 
		ALU::register_B.read(), ADR::getEffectiveAddress(), MON::INST, parityAlm);
	cout << buf << endl;

	sprintf_s(buf, 100, "      X:%06o   Y:%06o   U:%06o   (a)SA  :%1d", 
		ALU::register_X.read(), ALU::register_Y.read(), ALU::register_U.read(), MON::SA);
	cout << buf << endl;

	cout << endl;
	sprintf_s(buf, 100, "00    A:%06o   15   BANK:%02o       36  TIME1:%06o   53   OPT Y:%06o", 
		CRG::register_A.read(), ADR::register_BNK.read(), MEM::readMemory(036), MEM::readMemory(053));
	cout << buf << endl;
	sprintf_s(buf, 100,"01    Q:%06o   16 RELINT:%6s   37  TIME3:%06o   54  TRKR X:%06o", 
		CRG::register_Q.read(),"", MEM::readMemory(037), MEM::readMemory(054));
	cout << buf << endl;
	sprintf_s(buf, 100,"02    Z:%06o   17 INHINT:%6s   40  TIME4:%06o   55  TRKR Y:%06o", 
		CRG::register_Z.read(),"", MEM::readMemory(040), MEM::readMemory(055));
	cout << buf << endl;
	sprintf_s(buf, 100, "03   LP:%06o   20    CYR:%06o   41 UPLINK:%06o   56  TRKR Z:%06o", 
		CRG::register_LP.read(), MEM::readMemory(020), MEM::readMemory(041), MEM::readMemory(056));
	cout << buf << endl;

	sprintf_s(buf, 100, "04  IN0:%06o   21     SR:%06o   42 OUTCR1:%06o", 
