...
char const* SEQ::instructionString[] =
{
	"TC",
	"CCS",
	"INDEX",
...
char* SEQ::getControlPulses()
{
	...
	strcpy_s(buf, MAXPULSES*6, "");

	for(unsigned i=0; i<MAXPULSES && glbl_cp[i] != NO_PULSE; i++)
	{
		strcat_s(buf, MAXPULSES * 6, cpTypeString[glbl_cp[i]]);
		strcat_s(buf, MAXPULSES * 6, " ");
	}
